...
export function PublicProfilePage() {
  ...
  if (status === 'unauthorized') {
    return (
      <div style={containerStyle}>
        <CenteredCard>
          <h2 style={{ color: '#e0a090', margin: '0 0 12px' }}>需要登录</h2>
          <p style={{ color: '#a8a08e', margin: '0 0 18px' }}>
            CUA 玩家资料仅对已登录的成员开放。
          </p>
  ...
  if (status === 'not_found') {
    return (
      <div style={containerStyle}>
        <CenteredCard>
          <h2 style={{ color: '#e0a090', margin: '0 0 12px' }}>未找到玩家</h2>
          <p style={{ color: '#a8a08e', margin: '0 0 18px' }}>
            <code style={{ color: '#b8893a', background: 'rgba(184, 137, 58, 0.1)', padding: '2px 6px', borderRadius: 3 }}>
              {username ?? '(empty)'}
            </code>{' '}
            不存在或尚未设置资料。
          </p>
  ...
  if (status === 'private' && profile) {
    return (
      <div style={containerStyle}>
        <CenteredCard>
          <h2 style={{ color: '#a8a08e', margin: '0 0 12px' }}>🔒 此玩家资料私密</h2>
          <p style={{ color: '#8a8576', margin: '0 0 18px' }}>
            <strong style={{ color: '#f5f0e0' }}>{profile.display_name}</strong> 选择不公开资料。
          </p>
          <Link to="/" style={linkBtnStyle}>返回主页</Link>
        </CenteredCard>
      </div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <button
              ...
              style={{
                ...
                color: copyStatus === 'copied' ? '#7fc090' : '#a5c8ff',
                border: `1px solid ${copyStatus === 'copied'
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', gap: 20, alignItems: 'flex-start' }}>
            ...
            <div style={{
              ...
              {profile.avatar_url ? (
                ...
                <div style={{
                  ...
                  color: '#6e6856', fontSize: 32, fontFamily: 'serif',
                }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{
                ...
                color: '#f5f0e0', marginBottom: 4,
                letterSpacing: '0.02em',
              }}>
                {profile.display_name}
              </div>
              <div style={{
                fontSize: 13, color: '#8a8576',
          ...
          {followStats && (
            <div style={{
              ...
              <span>
                <strong style={{ color: '#f4a8c0', fontFamily: 'monospace', fontSize: 14 }}>
                  {followStats.following_count}
                </strong>
                <span style={{ marginLeft: 4 }}>关注</span>
              </span>
              <span>
                <strong style={{ color: '#a78bfa', fontFamily: 'monospace', fontSize: 14 }}>
          ...
          {profile.bio && (
            <div style={{
              ...
              borderLeft: '3px solid rgba(184, 137, 58, 0.4)',
              borderRadius: 2,
              fontSize: 13, color: '#d5cdb5',
              lineHeight: 1.7,
              fontStyle: 'italic',
            }}>
              "{profile.bio}"
          ...
          <Section title="链接">
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              {profile.links.map((link, i) => (
                ...
                >
                  <span style={{ fontWeight: 500 }}>{link.name || '(未命名)'}</span>
                  <span style={{ flex: 1, color: '#6e7a8a', fontSize: 11, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                    {link.url}
                  </span>
...
function Stat({ label, value }: { label: string; value: string }) {
  return (
    <div style={{
      ...
      <div style={{
        fontSize: 20, fontWeight: 600, color: '#f5f0e0',
        fontFamily: 'monospace',
      }}>
        {value}
      </div>
...
const tagStyle = (color: string, border: string): React.CSSProperties => ({
  padding: '3px 9px',
  background: `${color}15`,
